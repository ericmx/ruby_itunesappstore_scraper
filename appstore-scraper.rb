...
NATIVE_LANGUAGE = 'en'
file = File.open('./ruby_data.txt', 'a') #opens the output file or creates a new one if it doesn't already exist
...
software.keys.sort.each do |software_key|
  ...
  stores.sort_by { |a| a[:name] }.each do |store|
		$i = 0
		$num = 1 #adjust to grab more pages
		while $i < $num do
    ...
    if reviews.any?
      puts "=== Store: #{store[:name]}"
      	reviews.each_with_index do |review, index|
					file.write("#{review[:body]}") #this lie just writes the comment to the file, you can write additional itmes here instead, or do whatever you want.
